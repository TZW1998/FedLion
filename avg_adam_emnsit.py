...
parser.add_argument('--eps', type=float, default=1e-6, help='eps for Adam')
...
log_dir = 'logs/EMNIST/avgadamw_exp{}_bz{}_lr{}_b1{}_b2{}_eps{}_wd{}_nn{}_ls{}'.format(args.exp_index, batch_size, lr, beta1, beta2, eps, weight_decay, args.num_nodes, args.local_steps)
if os.path.exists(log_dir):
    clean_dir(log_dir)
logger = SummaryWriter(logdir= log_dir)
print('Start training with AVG-AdamW algorithm, batch size {}, local learning rate {}, betas ({},{}), eps {}, weight decay {}, num nodes {}, local steps {}.'.format(batch_size, lr, beta1, beta2, eps, weight_decay, args.num_nodes, args.local_steps))


global_weight = deepcopy(model.state_dict()) # be careful if batchnorm is used
global_state = {ni:{"step": 0,
                    "exp_avg" : torch.zeros_like(p),
                    "exp_avg_sq" : torch.zeros_like(p),
                    } 
                for ni, (_,p) in enumerate(global_weight.items())}

for com_round in range(args.total_steps):
    # Local SGD
    aggregated_models = {n: torch.zeros_like(p) for n, p in global_weight.items()}
    aggregated_states = {ni:{"step": 0,
                            "exp_avg" : torch.zeros_like(p),
                            "exp_avg_sq" : torch.zeros_like(p),
                            } 
                        for ni, (_,p) in enumerate(global_weight.items())}
    ...
    for client_ind, client in enumerate(choosed_clients):
        ...
        local_optimizer = optim.AdamW(model.parameters(), lr=lr, betas=(beta1, beta2), weight_decay=weight_decay,eps=eps)
