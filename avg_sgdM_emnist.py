...
log_dir = 'logs/EMNIST/avgsgd_exp{}_bz{}_lr{}_mm{}_wd{}_nn{}_ls{}'.format(args.exp_index, batch_size, lr, momentum, weight_decay, args.num_nodes, args.local_steps)
if os.path.exists(log_dir):
    clean_dir(log_dir)
logger = SummaryWriter(logdir= log_dir)
print('Start training with AVG-SGDwM algorithm, batch size {}, local learning rate {}, momentum {}, weight decay {}, num nodes {}, local steps {}.'.format(batch_size, lr, momentum, weight_decay, args.num_nodes, args.local_steps))
...
for com_round in range(args.total_steps):
    ...
    for client_ind, client in enumerate(choosed_clients):
        ...
        local_optimizer = optim.SGD(model.parameters(), lr=lr, momentum=momentum, weight_decay=weight_decay)
